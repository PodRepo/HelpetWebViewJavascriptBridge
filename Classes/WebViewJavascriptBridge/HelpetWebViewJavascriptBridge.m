//
//  HelpetWebViewJavascriptBridge.m
//  ExampleApp-iOS
//
...
#import "HelpetWebViewJavascriptBridge.h"
...
@implementation HelpetWebViewJavascriptBridge {
...
{
    HelpetWebViewJavascriptBridge* bridge = [[HelpetWebViewJavascriptBridge alloc] init];
...
- (void)initObjCHandler{
    
    NSString *messageJSON = [self _serializeMessage:[_messageHandlers allKeys]];
    [self _log:@"initObjC" json:messageJSON];
    ...
    NSString* javascriptCommand = [NSString stringWithFormat:@"window.HelpetWebViewJavascriptBridge_ObjCHandlerStr = '%@';", messageJSON];
...
- (void)dealloc {
    ...
    _messageHandler = nil;
    NSLog(@"HelpetWebViewJavascriptBridge dealloc");
}
...
- (void)_dispatchMessage:(WVJBMessage*)message {
    ...
    NSString* javascriptCommand = [NSString stringWithFormat:@"HelpetWebViewJavascriptBridge._handleMessageFromObjC('%@');", messageJSON];
    if ([[NSThread currentThread] isMainThread]) {
        [_webView stringByEvaluatingJavaScriptFromString:javascriptCommand];
    } else {
        __strong WVJB_WEBVIEW_TYPE* strongWebView = _webView;
...
- (void)_flushMessageQueue {
    NSString *messageQueueString = [_webView stringByEvaluatingJavaScriptFromString:@"HelpetWebViewJavascriptBridge._fetchQueue();"];
    
    id messages = [self _deserializeMessageJSON:messageQueueString];
    if (![messages isKindOfClass:[NSArray class]]) {
        NSLog(@"HelpetWebViewJavascriptBridge: WARNING: Invalid %@ received: %@", [messages class], messages);
        return;
    }
    for (WVJBMessage* message in messages) {
        if (![message isKindOfClass:[WVJBMessage class]]) {
            NSLog(@"HelpetWebViewJavascriptBridge: WARNING: Invalid %@ received: %@", [message class], message);
            continue;
        }
...
- (void)webViewDidFinishLoad:(UIWebView *)webView {
    ...
    if (_numRequestsLoading == 0 && ![[webView stringByEvaluatingJavaScriptFromString:@"typeof HelpetWebViewJavascriptBridge == 'object'"] isEqualToString:@"true"]) {
        [self initObjCHandler];
        NSBundle *bundle = _resourceBundle ? _resourceBundle : [NSBundle mainBundle];
        NSString *filePath = [bundle pathForResource:@"HelpetWebViewJavascriptBridge" ofType:@"js"];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([[url scheme] isEqualToString:kCustomProtocolScheme]) {
        if ([[url host] isEqualToString:kQueueHasMessage]) {
            ...
            NSLog(@"HelpetWebViewJavascriptBridge: WARNING: Received unknown HelpetWebViewJavascriptBridge command %@://%@", kCustomProtocolScheme, [url path]);
        }
        return NO;
